...
### Broad-judgment Pro fan-out (vs. agentic orchestrator loop) — 2026-05-17
- **Chose:** Keep Pro as a single broad-judgment call on the full transcript + retrieved context, emitting a coherent JSON of writes.
- **Passed on:** Decomposed agentic loop — Orchestrator + Context Explorer + Writer + Editor + per-artifact Specialists — driving mid-tier models in a multi-step loop.
- **Why:** The wiki capture decision is fundamentally **unified judgment, not fan-out**. A single transcript span often implies coupled decisions across multiple pages, scopes, slugs, and artifact kinds (e.g., *"I should ask Sarah about the kitchen reno before Tuesday"* = todo + person page + project page + scope routing + slug-parsimony check, all interdependent). Broad-judgment Pro holds the coupling in attention and commits a coherent set of writes; decomposed specialists each see a slice and the orchestrator reconciles, which is where multi-agent systems get *less* accurate. Also: (a) the model already runs an extensive internal reasoning loop pre-emission — forced inter-call serialization is lossy compression of latent state; (b) latency is load-bearing for next-call wiki freshness in a voice-first product; (c) a Context Explorer inside ingestion contradicts the locked `search-via-live-agent` separation (capture-time ≠ grounding-time); (d) the bet on "mid-tier × N beats Pro × 1" is a bet against the frontier curve — Pro improvements directly improve capture quality, orchestration scaffolding becomes legacy.
- **Gives up:** Per-call iteration / self-checking; cleaner per-artifact prompts (mitigated below); the wow-factor of visible multi-agent reasoning trails.
- **The piece worth borrowing — specialist tail for typed artifacts:** Decomposition genuinely wins where the work is independent and has hard schemas — todos, research outputs, calendar items, future structured uploads. Direction (not v0.3.0-scoped): **Pro decides intent + raw content + slug; small per-kind specialist calls fill structured shape.** Shrinks Pro's prompt (no longer carries every artifact schema), rides Pro improvements at the decision layer, lets specialists hold deeper per-kind nuance (e.g., todo recurrence inference, research scoping cues). Editor agent specifically *not* adopted — without verifiable criteria it just adds noise.
- **Revisit if:** Pro's prompt becomes unmanageable as new artifact kinds land (signal: artifact-schema bloat dominates Pro's token budget); or a specific artifact kind shows persistent shape-fidelity failures that a focused specialist would clearly fix.
...
### Full wiki tree in Live Agent preload (vs. depth-capped tree, vs. recency-biased slice)
- **Chose (2026-05-19):** Render the *entire* user-scope wiki tree — every non-tombstoned, non-archived page with its slug, title, type, and `agent_abstract` — into the Live Agent's call-time preload. No depth cap, no per-parent children limit. Single DB query, tree assembled client-side from `parent_page_id` pointers.
- **Passed on:** (a) Keeping the prior depth-2 cap (top-level + first-level children, plus a 12-child cap per parent). (b) A recency-weighted subset (e.g., last-touched N pages + their ancestors + top-level structure). (c) A flat slug-only index (no abstracts, no tree shape) as a lightweight reference.
- **Why:** Dogfood 2026-05-19 surfaced a recurring failure where Live Agent couldn't locate deeply-nested pages by name — said "hmm I can't find that" and waited for the user to coach it to the exact location. Two complementary fixes landed together: (1) prompt-side, a strengthened "search-with-reformulations before giving up" rule in `behavioral/tool-use.ts`; (2) preload-side, this full-tree expansion so Live Agent has every slug in context and can `fetch_page` directly without needing to guess or search at all for known pages. The full tree is cheap at current scale (Max's ~66 pages render to ~1.5k chars) and provides a complete navigational map.
- **Gives up:** Linear scaling in prompt size with page count. At the current scale the cost is negligible; at ~500 pages we're at ~12k chars (~3% of context); at ~5000 pages we're at ~120k chars and starting to compete with the rest of preload + behavioral guidance + grounding. Also gives up cache stability — every new page invalidates the prefix, hurting Gemini's explicit-prompt-cache hit rate on subsequent calls (mitigation parked: tighter cache prefix bounded by stable scaffolding, with the wiki tree below the cache cut).
- **Revisit if:** (a) Any user crosses ~500 pages and the preload starts feeling heavy (latency or token cost). First fallback: re-cap at depth-3 + recency-weighted leaves, OR switch to a flat slug-only index with abstracts dropped. (b) Cache-hit-rate telemetry shows the unstable wiki prefix hurting cost meaningfully — at that point the wiki tree moves below the cache cut, or graduates to a separately-fetched tool result. (c) The "search with reformulations" prompt rule reliably solves the page-location problem on its own; full-tree preload could revert to a leaner shape if the search side proves sufficient. Tracking: see v0.4.0 build phase notes for the dogfood pass that drove this decision.
