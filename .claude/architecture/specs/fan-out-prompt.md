...
**Second-iteration prompt fix (2026-05-18).** The first fix — sanitizing the worked example (synthetic UUIDs, prose-only ❌) — wasn't sufficient. With the full Pro audit captured via the `pro_fan_out_response` persistence shift, observed shape on retry: same KEEP-AS-IS + abstract-mention pattern, but now with the user's REAL `touched_pages` UUIDs (so Pro is no longer copying placeholders; it's freshly deriving the wrong shape). The deeper cause: the prompt's asymmetric framing — `agent_abstract` REQUIRED, `sections` OPTIONAL — taught the model that the abstract was the primary capture field. Pro emitted in 14s with high confidence, writing the new entity into the abstract and never touching section content.

Resolution: reframe `agent_abstract` semantically as a *derived summary OF section content*, not a capture field. Hard constraint added to the ontology primer + hard rules: every entity / claim named in `agent_abstract` MUST appear verbatim in at least one section's `content` field; write agent_abstract LAST, as a description of the section state. Additionally, called out explicitly in the Input section that **the live agent has no write tools** — when the transcript shows "I'll add X" / "Done, I added it", those are *assignments to Pro*, not evidence of completed work. Both were latent assumptions the prompt failed to make explicit.
