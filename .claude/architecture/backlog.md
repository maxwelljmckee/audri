...
| **Pro fan-out fork — Notes + Task Dispatch split** | **P2** | **M–L** | **Infra + Architecture** | Split the Pro fan-out inference into two specialized passes — one for note/wiki writes, one for action dispatch (todo creation, future agent-task spawns). Coordinated with Todo specialist maturation. **Deferred from v0.4.0 spec** (`customization-framework.md` Open Question D) pending traffic-director telemetry. Decision drivers: (a) does branch-distribution data show a meaningful task-dominant vs notes-dominant skew (>40% of standard-path calls), (b) does Todo specialist work surface concrete pain at the unified-Pro layer (e.g., task shapes diverging enough from notes that a single prompt becomes contradictory), (c) does coordination cost (parallel forks vs sequential, shared Flash candidates, cross-references between forks) feel tractable. Today's blocker: most real calls are mixed; splitting risks losing the cross-context that lets unified Pro decide that "email Sarah" should reference an existing Sarah page. Source: 2026-05-19 customization-framework spec session. |
