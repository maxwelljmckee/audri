...
| **Project-scoped todos (architecture decision needed)** | **P1** | **M** | **UX + Data model** | Today todos live in a flat hierarchy (`todos/<status>/<todo>`). As project work scales, a single flat list balloons. Proposed UX: each project gets its own todos namespace + the Todos plugin renders a tab strip (General, Project A, Project B, ...). Bringing back a top-level "General Todos" wiki page is part of this — explicitly RESERVED until the architecture decision below lands. **Architecture choice required:** (a) **status-as-hierarchy** (current pattern): each project gets nested status buckets like `projects/<slug>/todos/<status>/<todo>` — N projects × 4 buckets = many seeded pages per project. Doesn't scale. (b) **status-as-column** (preferred direction): individual todo pages carry `status` as a property (column on the page or sidecar), and the plugin groups by status within each tab. Cleaner long-term but a real schema refactor — needs `todo_status` enum, migration, ingestion-prompt updates, mobile UI rewrite for status grouping. (c) **hybrid**: project-scoped todo pages with current four-bucket structure replicated per project. Splits the difference. **Decide on (b) before implementing.** Other open questions: does each new project page auto-spawn a todo sub-page (server-side ingestion side-effect vs. lazy creation)? Should "General Todos" also follow status-as-column for consistency? **Source:** 2026-05-10 UX iteration; flagged for focused design pass. |
