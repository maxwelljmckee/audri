...
| **Add-page affordance in wiki UI** | **P1** | **S** | **UX + Feature** | Today the only way to create a new wiki/notes page is via voice (agent emits a new-page op during fan-out) or implicitly via ingestion. Add an explicit "+ New page" affordance available from any page in the Notes overlay (including top-level), so the user can manually create a page anywhere in the hierarchy. Design at spec time: parent picker default (current page as parent vs. sibling vs. user-picks), type picker (default `note`?), title-only quick-create vs. full editor on creation. Pairs with the broader CRUD UI entry above and with voice-driven wiki mutations (rename/tombstone in the prompt-engineering tranche). Source: post-MVP UX request 2026-05-10. |
...
| **Homescreen plugin tile icons: darken** | **P3** | **S** | **UX (polish)** | Plugin tile icons on the home screen render too light against the BlurView background — the glyphs wash out and don't feel grounded in the tile. Darken the icon color (or bump contrast / add subtle drop-shadow) so each glyph reads at a glance. Pure visual polish; no behavior change. Source: post-MVP UX request 2026-05-10. |
