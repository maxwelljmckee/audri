...
| Name | Priority | Effort | Type | Description |
|---|---|---|---|---|
| **Rename "Wiki" → "Notes" in UI** | **P0** | **S** | **UX (copy)** | All user-facing references to "Wiki" should read "Notes" — more approachable, less technical. Mechanical pass over the mobile codebase: PluginTile labels, PluginOverlay headers, any in-call mentions in copy or scaffolding that refer to the user's "wiki" (server-side prompts also affected — system-prompt.ts has "Wiki structure" headers, preload renders "Recently active wiki pages"; both need to change). Code identifiers (`wikiPages`, `useWikiPages`, `WikiSectionEditor`, `wiki-junctions.ts`, table names, etc.) stay as-is — the rename is *user-facing copy only*. Pair with the wiki cross-linking work (above) since both are about making the surface feel like a personal knowledge tool, not a database. Source: post-MVP UX request 2026-05-06. |
