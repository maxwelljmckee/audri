...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE public.call_transcripts (id, user_id, agent_id, session_id, call_type, kind, title, summary, started_at, ended_at, content, cancelled, end_reason, ingestion_status, ingestion_error, created_at, _deleted)';
  END IF;
