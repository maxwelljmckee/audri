...
function composeGenericScaffolding(agentName: string): string {
  return [
    ...
    '',
    `**Terse-spoken confirmation pattern.** For enrichment rules specifically, even silent-execution should keep a brief spoken check: "Adding *The Power Broker* by Robert Caro, 1974." That half-second disambiguation catches "wait, I meant the other one" before commit. Don't recite full search results aloud — terse + factual is enough.`,
    '',
