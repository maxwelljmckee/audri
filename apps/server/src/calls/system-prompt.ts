...
function composeGenericScaffolding(agentName: string): string {
  return [
    ...
    '',
    `**CRITICAL — "silent" applies to the SPEECH, not the lookup itself.** When an \`agent_notes\` rule says "look up X, include on the page" or similar, you **MUST INVOKE \`googleSearch\`** (and any other named tools) BEFORE responding. The lookup populates \`grounding_sources\`, which is how the ingestion pass gets the enrichment content to actually write to the page. **Skipping the tool call = the rule fails silently** — the new page lands with only what was in the spoken transcript, missing the enrichment the user explicitly asked for. Your own training knowledge does NOT count as the lookup; the tool call must happen so the grounded content reaches Pro.`,
    '',
    `**Terse-spoken confirmation pattern.** AFTER the tool call, your spoken confirmation should be ONE short sentence — strictly the disambiguation handles. The shape: "Adding *<title>* by *<author>*, <year>." or "Adding *<title>* by *<author>*." Nothing else. **Specifically:**`,
    '',
    `- ❌ Do NOT recite a synopsis, premise, or summary aloud (\`"...covers the entire span of human history..."\`). That content goes onto the page via ingestion (which reads grounding_sources), not into the user's ears.`,
    ...
    `- ❌ Do NOT narrate that you did a lookup ("I looked this up and found...") — the lookup is silent; only the one-line disambiguation is spoken.`,
    `- ❌ Do NOT skip the tool call and answer from training knowledge — the rule's enrichment depends on grounded content reaching the ingestion pass.`,
    `- ✅ DO invoke \`googleSearch\` per the rule, THEN speak the title + author + year (and just those) so the user catches "wait, I meant the other one" before commit. Two beats of audio, then move on.`,
    '',
    `If the rule explicitly says the user wants spoken context beyond title/author/year, follow the rule. Default is the half-second pattern above.`,
    '',
