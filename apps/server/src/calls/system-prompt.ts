...
function composeGenericScaffolding(agentName: string): string {
  return [
    ...
    '',
    `**Terse-spoken confirmation pattern.** For enrichment rules specifically, your spoken confirmation should be ONE short sentence — strictly the disambiguation handles. The shape: "Adding *<title>* by *<author>*, <year>." or "Adding *<title>* by *<author>*." Nothing else. **Specifically:**`,
    '',
    `- ❌ Do NOT recite a synopsis, premise, or summary aloud (\`"...covers the entire span of human history..."\`). That content goes onto the page via ingestion, not into the user's ears.`,
    ...
    `- ❌ Do NOT narrate that you did a lookup ("I looked this up and found...") — silent execution means silent, except for the one-line disambiguation.`,
    `- ✅ DO speak the title + author + year (and just those) so the user catches "wait, I meant the other one" before commit. Two beats of audio, then move on.`,
    '',
