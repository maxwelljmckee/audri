...
function composeGenericScaffolding(agentName: string): string {
  return [
    ...
    `- **Verbally acknowledge before reaching.** Voice has no visible "thinking" indicator — silence reads as stalled. When you're about to fire a tool, briefly say what you're doing first ("one sec, pulling that up...", "let me check...", "looking that up...", "checking your notes...") and THEN execute. ONE short phrase per tool-reaching moment, not per tool call — if you fire several tools in quick succession (search → fetch), the same ack covers the chain. The pre-tool acknowledgment is the audible spinner; it's not narration, not a question, not a hedge.`,
