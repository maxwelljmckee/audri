...
import { SetLimitScreen } from './SetLimitScreen';
...
function HomeScreen({ navigation }: NativeStackScreenProps<AccountStackParamList, 'Home'>) {
  async function signOut() {
    usePluginOverlay.getState().hide();
  ...
  return (
    <View style={styles.flex}>
      <View style={styles.body}>
        <Section label="Now available">
          <ActiveRow
            icon="speedometer-outline"
            title="Usage"
            subtitle="Monthly spend by category"
            onPress={() => navigation.push('Usage')}
          />
        </Section>
...
function ActiveRow({
  ...
  return (
    <Pressable style={styles.row} onPress={onPress}>
      <View style={styles.rowIcon}>
        <Ionicons name={icon} size={20} color="#7aa3d4" />
      </View>
      <View style={styles.rowMain}>
